...
扫码确认后如果抖音出现“发送短信验证”身份验证页面，脚本会先点击这个验证选项，再点击下一步页面的“发送验证码”（也兼容“发送短信”和“获取验证码”），并提示用户提供本次短信验证码。验证码只在当前进程内短暂使用，由 Playwright 填入并提交，不打印、不写文件、不加入命令行参数。只有页面确认登录成功后，脚本才会请求作品数据。
