...
扫码确认后如果抖音出现身份验证选项，脚本优先点击“接收短信验证码”（兼容“发送短信验证”），再点击下一步页面的“发送验证码”（也兼容“发送短信”和“获取验证码”）；如果验证码输入框已直接出现，则直接进入填码流程。随后提示用户提供本次短信验证码，填入后自动点击确认/登录按钮。验证码只在当前进程内短暂使用，不打印、不写文件、不加入命令行参数。只有页面确认登录成功后，脚本才会请求作品数据。
