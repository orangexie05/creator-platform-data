...
扫码确认后，如果抖音出现短信验证页面，采集器会自动点击一次“发送短信”；如果页面使用“发送验证码”或“获取验证码”，也应点击对应发送动作。随后保持当前浏览器页面等待用户完成验证。提示用户直接在打开的抖音浏览器页面输入验证码，不要要求用户把验证码发到聊天、命令行参数或日志中。只有页面登录状态确认成功后，才请求作品接口。
...
- 等待扫码时保持浏览器停留在登录页。
- 扫码后若出现短信验证，默认只发送一次短信，并等待用户在浏览器页面完成验证码验证。
